...
kpS = 28 # proportional gain for distance error
kpR = 15 # proportional gain for angle error

kiS = 3 # integral gain for distance error
kiR = 3 # integral gain for angle error

kdS = 7 # differential gain for distance error
kdR = 5 # differential gain for angle error
...
def run(curPosition, goal, tetta):
	...
	dataOutput = open("4LabPIDMeasurements [" + str(round(curPosition[0])) + " " + str(round(curPosition[1])) + "] -> [" + str(goal[0]) + " " + str(goal[1]) + "].txt","w+")
	...
	while distanceToGoal > 0.03:
		...
		# uSI - integral component of voltage for linear speed
		...
		uS = kpS * distanceToGoal + uSI + kdS * (distanceToGoal - prevDistanceToGoal) / dt
		...
		uR = kpR * headingAngle + uRI + kdR * (headingAngle - prevHeadingAngle) / dt
