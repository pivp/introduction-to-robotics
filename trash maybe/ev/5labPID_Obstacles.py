...
kiS = 6 # integral gain for linear speed
kiR = 5 # integral gain for angular speed

kdS = 7 # differential gain for linear speed
kdR = 4 # differential dain for angular speed

motorR = LargeMotor('outA') # right motor declaration
motorL = LargeMotor('outB') # left motor declaration

sensorR = UltrasonicSensor("in1") # right us sensor declaration
sensorL = UltrasonicSensor("in3") # left us sensor declaration

# stopButton = TouchSensor("in1") # robot's stop button declaration

minDist = 0.15 # min distance to avoid obstacle
kObs = 30 # coefficient to form an obstacle avoidance angle
...
def run(curPosition, goal, tetta):
	...
	dataOutput = open("5LabPID_ObstaclesMeasurements [" + str(round(curPosition[0])) + " " + str(round(curPosition[1])) + "] -> [" + str(goal[0]) + " " + str(goal[1]) + "].txt","w+")
...
curPosition, tetta = run([0, 0], [5, 0], 0)